...
export function formFromUser(user: SessionUser) {
  ...
  return {
    ...
    gender: p.gender ?? "",
    phone: user.phone ?? "",
    // Step 2 — Address. The permanent trio is the one the NID checks compare
    // against, which is why the district is picked from a list rather than
    // typed: a free-text district can't be matched to a postcode block.
    ...
    permanentPostcode: p.permanentPostcode ?? "",
    nid: p.nid ?? "",
...
export function toProfile(form: WizardForm): ProfessionalProfile {
  return {
    ...
    currentPostcode: form.currentPostcode || undefined,
    permanentAddress: form.permanentAddress || undefined,
    permanentDivision: form.permanentDivision || undefined,
    permanentDistrict: form.permanentDistrict || undefined,
    permanentPostcode: form.permanentPostcode || undefined,
