...
/**
 * The public profile for one of the three searchable professions. The
 * `/architects/[id]`, `/engineers/[id]` and `/contractors/[id]` pages are thin
 * wrappers around this — `role` picks the back-link, "not found" copy, and the
 * Contact section's wording from {@link professionalCopy}.
 *
 * "Book a meeting" stays architect-only: it's backed by a separate
 * availability/slots system built for the architect's booking flow, not a
 * generic professional-scheduling feature.
 */
export function ProfessionalDetail({ role }: { role: UserRole }) {
  ...
  async function handleContact(e: React.FormEvent) {
    ...
    setSending(true);
    try {
      await createInquiry(token, { professionalId: params.id, message });
      setSent(true);
    } catch (err) {
  ...
  // Unverified professionals are browsable but can't be engaged. The API
  // enforces this in createInquiry; hiding the form here just avoids a
  // pointless 403.
  const isVerified = professional?.verificationStatus === VerificationStatus.APPROVED;

  if (loading || notFound || !professional) {
    return (
  ...
  const projects = professional.portfolio ?? [];
  const awards = professional.achievements ?? [];
  ...
  const expertise = professional.expertise?.length
      ...
      <main className="flex-1 px-5 pt-28 pb-16 sm:px-8">
        <div className="mx-auto w-full max-w-6xl">
          ...
          {/* ---- Hero: the professional's own statement left, portrait fading
               in from the right — the photo's left edge dissolves into the
          ...
          <section className="mt-8 grid gap-8 lg:grid-cols-2 lg:items-center">
            <div className="relative z-10 lg:py-10">
              ...
              {(professional.portfolioIntro || professional.bio) && (
                <p className="mt-6 max-w-md text-[15px] leading-relaxed text-stone-600 dark:text-slate-400">
                  {professional.portfolioIntro ||
                    (professional.bio!.length > 180
                      ? `${professional.bio!.slice(0, 180).trimEnd()}…`
                      : professional.bio)}
                </p>
              )}
              ...
              <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2">
                <Stars rating={professional.ratingAvg} count={professional.ratingCount} size="lg" />
                {professional.practiceDistrict && (
                  <span className="text-sm text-stone-600 dark:text-slate-400">
                    Practises in {professional.practiceDistrict}
                    {professional.practiceDivision && `, ${professional.practiceDivision}`}
                  </span>
                )}
              </div>

              {(typeof professional.yearsExperience === "number" ||
                projects.length > 0 ||
                awards.length > 0) && (
                <div className="mt-10 flex flex-wrap gap-x-10 gap-y-6 border-t border-stone-200 pt-8 dark:border-white/10">
                  {typeof professional.yearsExperience === "number" && (
                    <Stat value={`${professional.yearsExperience}+`} label="Years" />
                  )}
            ...
            {professional.avatarUrl ? (
              <figure className="relative lg:-ml-24">
                ...
                <figcaption className="absolute top-1/2 right-5 z-10 max-w-[45%] -translate-y-1/2 text-white [text-shadow:0_1px_14px_rgba(0,0,0,0.55)]">
                  ...
                  <div className="mt-2.5 flex flex-wrap items-center gap-2">
                    ...
                    <PendingBadge status={professional.verificationStatus} />
                  </div>
                </figcaption>
              </figure>
            ) : (
              <figure className="relative overflow-hidden rounded-3xl">
                ...
                <figcaption className="absolute right-0 bottom-0 left-0 bg-linear-to-t from-black/60 to-transparent p-5 text-white">
                  <p className="text-lg font-extrabold">{professional.name}</p>
                  {(professional.company || professional.professionalTitle) && (
                    <p className="text-sm text-white/80">
                      {[professional.professionalTitle, professional.company]
                        .filter(Boolean)
                        .join(" · ")}
                    </p>
                  )}
          ...
          <section className="mt-16 grid gap-10 border-t border-stone-200 pt-10 sm:grid-cols-2 lg:grid-cols-3 dark:border-white/10">
            <div>
              ...
              {professional.bio ? (
                ...
                <p className="mt-3 text-sm text-stone-500 dark:text-slate-500 capitalize">
                  This {copy.singular} hasn&apos;t written a bio yet.
                </p>
              )}
              {professional.licenseAuthority && (
                <p className="mt-4 text-sm font-semibold">
                  Registered with {professional.licenseAuthority}
                </p>
              )}
            ...
            {(typeof professional.yearsExperience === "number" ||
              ...
              <div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-x-6 gap-y-7">
                  {typeof professional.yearsExperience === "number" && (
                    <Stat value={`${professional.yearsExperience}+`} label="Years of experience" />
                  )}
                  {projects.length > 0 && (
          ...
          {/* ---- Reviews ---- Only real ones: each comes from a land owner
               whose contract with this professional reached COMPLETED. */}
          {reviews.length > 0 && (
          ...
          {/* ---- Book a meeting ---- Architect-only; see the component doc. */}
          {role === UserRole.ARCHITECT && isVerified && (
            <section className="mt-16 border-t border-stone-200 pt-10 dark:border-white/10">
              ...
              <BookMeeting architectId={params.id} architectName={professional.name} />
            </section>
          )}
            ...
            <div className="rounded-2xl border border-white/50 bg-white/55 p-6 shadow-xl shadow-black/5 backdrop-blur-xl sm:p-10 dark:border-white/10 dark:bg-white/5">
              <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
                <div>
                  ...
                  <p className="mt-2 text-sm text-stone-600 dark:text-slate-400">
                    {copy.contactBody.replace("{firstName}", firstName)}
                  </p>
                </div>
              </div>
              ...
              {sent ? (
                ...
                <div className="mt-6 rounded-xl bg-amber-100 px-4 py-3.5 text-sm text-amber-900 dark:bg-amber-400/15 dark:text-amber-200">
                  ...
                  <p className="mt-1 capitalize">
                    You can look through their work, but requests can only be sent to {copy.plural}{" "}
                    whose credentials and identity a Buildora supervisor has checked. Come back once
                    they&apos;re verified, or{" "}
                    <Link href={`/${copy.basePath}`} className="underline underline-offset-2">
                      browse verified {copy.plural}
                    </Link>
                    .
                  </p>
                </div>
