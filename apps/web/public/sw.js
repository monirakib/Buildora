...
self.addEventListener("push", (event) => {
  ...
  const options = {
    body: payload.body || "",
    // Both generated by scripts/generate-icons.mjs from the navbar's own mark.
    // `icon` is the full-colour tile shown in the notification itself; `badge`
    // is the small monochrome one Android puts in the status bar, which uses
    // only the alpha channel — which is why it is a white glyph on transparent
    // rather than the amber-on-ink tile.
    icon: "/icon-192.png",
    badge: "/badge-72.png",
    //
